...
from database.users.repository.abstract import AbstractUsersRepository
...
class SQLAlchemyUsersRepository(AbstractUsersRepository):
    ...
    async def get_user_by_id(self, user_id: int) -> UserSchema:
        async with self._async_session_factory() as session:
            ...
            return user.scalar_one().to_pydantic_model()
    ...
    async def set_last_publication_time_by_user_id(
        ...
        async with self._async_session_factory() as session:
            ...
            await session.execute(stmt)
            await session.commit()
            return {"message": f"last publication time for user with id {user_id} set to {last_publication_time}"}
